...
contract MockClient is ILightClient {
    ...
    ) external view virtual override returns (uint64, bool) {
    ...
    function getLatestHeight(
        string calldata clientId
    ) external view virtual override returns (Height.Data memory, bool) {
    ...
    function getStatus(
        string calldata clientId
    ) external view virtual override returns (ClientStatus) {
    ...
    function verifyMembership(
        ...
        require(
            keccak256(IIBCHandler(ibcHandler).getCommitmentPrefix()) ==
                keccak256(prefix),
            "invalid prefix"
        );
    ...
    function getClientState(
        string calldata clientId
    ) external view virtual returns (bytes memory clientStateBytes, bool) {
        ClientState.Data storage clientState = clientStates[clientId];
    ...
    function getConsensusState(
        string calldata clientId,
        Height.Data calldata height
    ) external view virtual returns (bytes memory consensusStateBytes, bool) {
        ConsensusState.Data storage consensusState = consensusStates[clientId][
            height.toUint128()
        ];
